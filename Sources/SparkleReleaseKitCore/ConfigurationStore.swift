...
public struct ConfigurationStore: Sendable {
    ...
    public func load(from url: URL) throws -> SparkleKitConfiguration {
        ...
        let configuration = try JSONDecoder().decode(SparkleKitConfiguration.self, from: data)
        guard configuration.schemaVersion == SparkleKitConfiguration.currentSchemaVersion else {
    ...
    public func validate(_ configuration: SparkleKitConfiguration, allowMissingPublicKey: Bool = false) throws {
        ...
        guard !configuration.app.name.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty,
              ...
              !containsControlCharacter(configuration.app.name) else {
        ...
        guard let feedURL = URL(string: configuration.updates.feedURL),
              ...
              URLComponents(url: feedURL, resolvingAgainstBaseURL: false)?.fragment == nil else {
        ...
        guard matches(configuration.github.owner, #"^[A-Za-z0-9](?:[A-Za-z0-9-]{0,38})$"#),
              ...
              ![".", ".."].contains(configuration.github.repository) else {
        ...
        guard isSafeRelativePath(configuration.project.container),
              ...
              ["xcodeproj", "xcworkspace"].contains(URL(fileURLWithPath: configuration.project.container).pathExtension.lowercased()) else {
        ...
        guard !configuration.project.scheme.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty,
              ...
              !containsGitHubExpression(configuration.project.configuration) else {
    ...
    private func validateRawDocument(_ data: Data) throws {
        ...
        try requireObject(root["updates"], keys: ["sparkleVersion", "feedURL", "publicEDKey", "automaticChecks", "automaticDownloads", "channel"], path: "updates")
        try requireObject(root["distribution"], keys: ["installer", "updateArchive", "notarization"], path: "distribution")
    }
    ...
    private func requireOnly(_ dictionary: [String: Any], keys: Set<String>, path: String) throws {
        ...
        guard unknown.isEmpty else {
            let suspicious = unknown.filter {
                ...
                return key.contains("private") || key.contains("secret") || key.contains("token") || key.contains("password") || key.contains("certificate")
            }
